package org.zerock.mallapi.domain.admin.repository;
...
public interface BannedRepository extends JpaRepository<Banned, Long> {
    ...
    // 특정 관리자가 조치한 정지 내역 조회
    @Query("SELECT b FROM Banned b WHERE b.bannedBy = :adminId ORDER BY b.bannedAt DESC")
    List<Banned> findBansByAdmin(@Param("adminId") Long adminId);

    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(auth -> {
            auth.requestMatchers("/api/member/login").permitAll() // 로그인 요청 허용
                ...
                .requestMatchers("/api/member/verify-password").permitAll() // 비밀번호 확인 허용
                .requestMatchers("/api/member/withdraw").permitAll() // 회원탈퇴 허용
                ...
                .requestMatchers(GET, "/api/board/*").permitAll() // 게시글 상세 조회 (ID는 숫자만)
                .requestMatchers("/api/board/*/replies/**").authenticated() // 댓글 전체 인증 필요
        ...
        http.addFilterBefore(new JWTCheckFilter(),
                UsernamePasswordAuthenticationFilter.class); //JWT 체크        //@PreAuthorize 와 같은 접근 제한 어노테이션에서
